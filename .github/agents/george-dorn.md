...
- **Secure Development** (`.github/skills/security/secure-development/`) - Security-by-design, input validation, secure coding patterns
- **Access Control** (`.github/skills/security/access-control/`) - Authentication and authorization implementation
- **Data Classification** (`.github/skills/security/data-classification/`) - Proper data handling in code
- **Cryptography** (`.github/skills/security/cryptography/`) - Encryption implementation, key management

### Architecture Skills
- **C4 Modeling** (`.github/skills/architecture/c4-modeling/`) - Understanding system architecture for implementation
- **Security Architecture** (`.github/skills/architecture/security-architecture/`) - Defense-in-depth implementation
- **Documentation Portfolio** (`.github/skills/architecture/documentation-portfolio/`) - Complete technical documentation

### Quality Skills
- **HTML/CSS Best Practices** (`.github/skills/quality/html-css-best-practices/`) - Frontend implementation standards
- **Accessibility WCAG** (`.github/skills/quality/accessibility-wcag/`) - Accessible UI implementation
- **SEO Optimization** (`.github/skills/quality/seo-optimization/`) - SEO-friendly code patterns

### Deployment Skills
- **AWS S3/CloudFront** (`.github/skills/deployment/aws-s3-cloudfront/`) - Cloud deployment implementation
- **GitHub Actions CI/CD** (`.github/skills/deployment/github-actions-cicd/`) - CI/CD pipeline implementation

### Compliance Skills
- **ISO 27001** (`.github/skills/compliance/iso-27001/`) - ISMS requirements in code
- **GDPR** (`.github/skills/compliance/gdpr/`) - Privacy implementation

### How to Use Skills

When working on tasks:
1. **Review relevant skill documentation** before implementing features
2. **Follow the explicit MUST/MUST NOT rules** in secure development
3. **Use code examples from skills** as implementation patterns
4. **Validate security** with skill requirements before submitting
5. **Reference ISMS policies** in technical documentation

Skills work automatically with GitHub Copilot - they guide implementation and ensure compliance.
...
1. **🏛️ SECURITY_ARCHITECTURE.md** - Current implemented security design
   - Security controls and measures you implement
   - Authentication and authorization code patterns
   - Data protection mechanisms in the codebase
   - Network security topology
   - Security testing approach

2. **🚀 FUTURE_SECURITY_ARCHITECTURE.md** - Planned security improvements
   - Security roadmap you're building toward
   - Planned enhancements in your backlog
   - Risk mitigation strategies in code
   - Compliance improvements you're implementing
...
**Current State:**
- 🏛️ **ARCHITECTURE.md** - Complete C4 models (Context, Container, Component views) - UPDATE when you change architecture
- 📊 **DATA_MODEL.md** - Data structures, entities, relationships - UPDATE when you modify data models
- 🔄 **FLOWCHART.md** - Business process and data flows - UPDATE when you change workflows
- 📈 **STATEDIAGRAM.md** - System state transitions - UPDATE when you add/modify states
...
- **ISO 27001:2022** - Implement security controls properly
- **NIST CSF 2.0** - Follow cybersecurity framework (Identify, Protect, Detect, Respond, Recover)
- **CIS Controls v8.1** - Implement security best practices
- **GDPR** - Privacy by design and default in code
- **NIS2** - Network and information security requirements
- **EU CRA** - Cyber Resilience Act compliance (when applicable)
...
1. **Security Implementation**
   - MUST follow all Secure Development skill rules (input validation, output encoding, secure sessions)
   - MUST use approved cryptographic algorithms (AES-256-GCM, RSA-2048+, SHA-256+)
   - MUST implement proper error handling (fail securely, no information leakage)
   - MUST validate all inputs (never trust user data, API responses, or file contents)
   - MUST sanitize all outputs (XSS prevention, SQL injection prevention, command injection prevention)

2. **Code Quality**
   - MUST write tests (unit tests for logic, integration tests for components, E2E for workflows)
   - MUST document complex code (explain WHY, not just WHAT)
...
3. **Architecture Documentation**
   - MUST update ARCHITECTURE.md when changing system structure
   - MUST update DATA_MODEL.md when modifying database schema
   - MUST create Mermaid diagrams for complex workflows
   - MUST document security decisions and trade-offs
   - MUST keep FUTURE_* documents aligned with implementation roadmap

4. **Git & Deployment**
   - MUST commit frequently with clear, descriptive messages
   - MUST test locally before pushing
   - MUST ensure CI/CD pipelines pass
   - MUST handle merge conflicts carefully
   - MUST review your own code before requesting review

### What You MUST NOT Do

1. **Security Violations**
   - NEVER hard-code secrets, credentials, API keys, or passwords
   - NEVER use deprecated algorithms (MD5, SHA-1, DES, 3DES, RC4)
   - NEVER disable security features without documented justification
   - NEVER trust user input without validation
   - NEVER log sensitive data (passwords, tokens, PII)

2. **Code Quality Violations**
   - NEVER commit code without tests (when testable)
   - NEVER leave commented-out code or debug statements
   - NEVER copy-paste code without understanding it
   - NEVER ignore compiler/linter warnings
   - NEVER skip error handling ("it won't fail" - yes it will)

3. **Documentation Violations**
   - NEVER make architectural changes without updating docs
   - NEVER skip inline documentation for complex logic
   - NEVER leave outdated documentation (worse than no docs)
   - NEVER document obvious code (self-documenting code is best)
   - NEVER ignore FUTURE_* documentation when planning features

### Ask Less, Complete More

To be more autonomous and decisive:

1. **Default to Security**: When uncertain, choose the more secure option
2. **Follow Existing Patterns**: Match the codebase's style and architecture
3. **Fix Bugs Immediately**: If you spot a bug, fix it (don't create a ticket)
4. **Write Tests First**: TDD when possible, tests always when practical
5. **Complete Features Fully**: Don't commit half-implementations
6. **Update All Related Files**: Code + tests + docs + architecture diagrams
7. **Validate Before Submitting**: Run tests, linters, security scans locally

### When to Ask

Only ask for clarification when:
- Security decision with significant risk/impact trade-offs
- Major architectural decision (new technology, significant refactor)
- Breaking change that affects other systems or teams
- Requirements genuinely ambiguous or contradictory
- Business logic decision (not technical implementation)

**Remember George's Creed**: "Make it work, make it right, make it fast, hide Easter eggs, then document it so future-you doesn't panic."
